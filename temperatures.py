...
class Tpoints:
    def __init__(self, v0, v6, va, dt=6.0, day=0):
        self.day = day  # day of month
    ...
    def __repr__(self):
        return ("v0: %.2f, v6: %.2f, va: %.2f, dt: %.2f, tau: %.3f"
                % (self.v0, self.v6, self.va, self.dt, self.tau))

    def get_tau(self):
        return self.tau

    def get_day(self):
        return self.day  # 0 if error, 1..31 otherwise

    def get_tpoint(self):
        return ("%.f\t(%.2f; %.2f; %.2f; %.2f)"
                % (self.tau, self.v0, self.v6, self.va, self.dt))
    ...
    def add_points(self):
        ...
        for s in self.datas:  # s is a sample
            if s.mm == 0:  # past midnight, first measure
                if (s.dd > self.day+1):
                    print("Warning: lack of data, days %d - %d" % (self.day, s.dd))
                self.day = s.dd     # copy day nr
                self.v0 = s.t_medium
                continue
            ...
            if s.mm == 6 * 60:
                if self.day == s.dd:
                    if s.dd == self.day:  # last sample time; compute the Tpoints
                        self.v6 = v6_min
                        self.t_cst = Tpoints(self.v0, self.v6, self.va, self.dt, self.day)
                        Temp.cst.append(self.t_cst)  # store as day value
                        # print("day:%d - %s" % (self.day, self.t_cst.__repr__()) )
                        Temp.nb_days = Temp.nb_days + 1
                else:
                    print("Warning: day %d - lack of data!" % self.day)
